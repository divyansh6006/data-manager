...
// Upload Data & Distribute are deliberately absent here — team leaders get supervision powers
// (team overview, reporting, transfers/approvals) only; bulk data import and candidate
// distribution are Super Admin-only, mirroring the Admissions team_leader role exactly
// (backend/src/index.js's /api/leads/upload and /api/leads/distribute never allowed
// team_leader either). Enforced server-side too — see candidates.routes.js's
// requireRole(['super_admin']) on /candidates/upload, /candidates/upload-preview,
// /upload-batches, and /candidates/distribute.
const NAV_LABELS = {
  dashboard: 'Dashboard', candidates: 'Candidates',
  recruiters: 'Recruiters', transfers: 'Transfer Approvals',
  interviews: 'Interview Schedule', callbacks: 'Callbacks', analytics: 'Analytics', settings: 'Settings'
};
...
export default function HiringTeamLeaderDashboard({ token, user, onLogout }) {
  ...
  // Live polling so the sidebar badge reflects new requests without a manual refresh,
  // mirroring the Admissions Team Leader's Transfer Approvals badge.
  useEffect(() => {
  ...
  const sections = [
    { title: 'Workspace', items: [
      { key: 'dashboard', label: 'Dashboard', icon: 'dashboard', sublabel: 'Team overview', onClick: () => setActiveSection('dashboard') },
      { key: 'candidates', label: 'Candidates', icon: 'group', sublabel: "Your team's candidates", onClick: () => setActiveSection('candidates') },
      { key: 'recruiters', label: 'Recruiters', icon: 'badge', sublabel: 'Manage your team', onClick: () => setActiveSection('recruiters') },
    ...
    <div className={isDark ? 'dark' : ''}>
      <div className="h-screen overflow-hidden bg-[#F9F9FF] dark:bg-slate-950 flex text-[#111C2D] dark:text-slate-100">
        ...
        <main className="flex-grow p-6 overflow-y-auto space-y-6 animate-fade-in" key={activeSection}>
          <div className="mb-2">
            <span className="text-[10px] uppercase tracking-widest text-slate-400 font-label-caps">Hiring Team Leader Portal</span>
            <h1 className="text-lg font-bold text-[#111C2D] dark:text-white font-headline-md">{NAV_LABELS[activeSection]}</h1>
          </div>

          {activeSection === 'dashboard' && <DashboardHome token={token} scope={scope} showToast={showToast} user={user} role="hiring_team_leader" onNavigateToCandidates={goToCandidatesWithStatus} />}
          {activeSection === 'candidates' && <CandidatesPage token={token} role="hiring_team_leader" scope={scope} showToast={showToast} searchOverride={searchOverride} statusFilterOverride={statusFilterOverride} />}
          {activeSection === 'recruiters' && <RecruitersPage token={token} role="hiring_team_leader" showToast={showToast} />}
          {activeSection === 'transfers' && <TransferApprovalsPage token={token} showToast={showToast} />}
          {activeSection === 'interviews' && <InterviewsPage token={token} role="hiring_team_leader" showToast={showToast} />}
